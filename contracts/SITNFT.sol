...
contract SITNFT is ERC721, ERC721Enumerable, RoleControl {
     ...
     * - The caller must be an admin.
     */
    function multiAddStudentAddress(IdAddress[] calldata _array)
        external
        onlyAdmin
    {
     ...
     *
     */
    function _getStudentAddress(string memory _id)
        private
        view
        returns (address)
    {
        bytes32 encryptedId = keccak256(abi.encodePacked(_id));
        require(
            _studentAddress[encryptedId] != address(0),
    ...
    function burn(uint256 tokenId) external onlyFaculty {
        ...
        require(
            msg.sender == _attributes[tokenId].faculty,
            "Only faculty who minted this token can burn it."
        );
        ...
        override(ERC721, IERC721)
    {
        ...
        require(
            to == _attributes[tokenId].faculty,
            "Only faculty who minted this token can be approved."
        );
